...
directory = r'/lustre/ahome3/n/nm416/Desktop/code2/h5lst'
#each filename ending in .h5 in this directory is added to name array
for filename in os.listdir(directory):
    if filename.endswith(".h5"):
        name.append(os.path.join('h5lst/'+filename))
    else:
        continue
...
x = dates
y = monthlyAverage
ax.errorbar(x,y, yerr = monthlystd, fmt = '-', capsize = 3, color = ('xkcd:rust'), 
            ecolor = 'grey',  linewidth=1)

plt.tight_layout()
ax.plot(dates,monthlyAverage, color = ('xkcd:rust'), marker = '.')  # Plot some data on the axes.
ax.xaxis.set_major_locator(plt.MaxNLocator(12))
ax.set_ylim(270,310)
plt.xlabel("Month and Year")
plt.ylabel("LST (Kelvin)")
plt.title("LST of the Shanxi Province from 2010 to 2021 (SD)")
...
plt.tight_layout()
plt.savefig("Shanxi_std", dpi = 400)
